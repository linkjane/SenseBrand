...
export class SolutionService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/solutions';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/solutions';
    ...
    create(solution: Solution): Observable<EntityResponseType> {
        ...
        if (filePostArr.length <= 0) {
            return this.http.post<Solution>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(solution: Solution): Observable<EntityResponseType> {
        ...
        if (filePostArr.length <= 0) {
            return this.http.put<Solution>(this.resourceUrl, copy, { observe: 'response' })
                .map((res: EntityResponseType) => this.convertResponse(res));
        }
        return forkJoin(...filePostArr).concatMap((results: FileCallbackModel[]) => {
            results.forEach((element: FileCallbackModel) => {
    ...
    search(req?: any): Observable<HttpResponse<Solution[]>> {
        const options = createRequestOption(req);
        return this.http.get<Solution[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Solution[]>) => this.convertArrayResponse(res));
    }

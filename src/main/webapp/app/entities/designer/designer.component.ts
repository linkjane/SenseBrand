...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.designerService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<Designer[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/designer'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Designer) {
        return item.id;
    }
    ...
    registerChangeInDesigners() {
        this.eventSubscriber = this.eventManager.subscribe('designerListModification', (response) => this.loadAll());
    }

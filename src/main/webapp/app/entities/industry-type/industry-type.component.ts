...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.industryTypeService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<IndustryType[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/industry-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IndustryType) {
        return item.id;
    }
    registerChangeInIndustryTypes() {
        this.eventSubscriber = this.eventManager.subscribe('industryTypeListModification', (response) => this.loadAll());
    }

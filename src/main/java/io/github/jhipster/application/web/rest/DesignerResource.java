...
public class DesignerResource {
    ...
    public ResponseEntity<List<Designer>> getAllDesigners(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("ideadetails-is-null".equals(filter)) {
            log.debug("REST request to get all Designers where ideaDetails is null");
            return new ResponseEntity<>(designerService.findAllWhereIdeaDetailsIsNull(),
                    HttpStatus.OK);
        }
        if ("show-is-null".equals(filter)) {
            log.debug("REST request to get all Designers where show is null");
            return new ResponseEntity<>(designerService.findAllWhereShowIsNull(),
                    HttpStatus.OK);
        }

...
public class DesignerResource {
    ...
    public DesignerResource(DesignerService designerService) {
    ...
    public ResponseEntity<Designer> createDesigner(@Valid @RequestBody Designer designer) throws URISyntaxException {
        ...
        Designer result = designerService.save(designer);
    ...
    @Timed
    public ResponseEntity<List<Designer>> getAllDesigners(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("designerideadetails-is-null".equals(filter)) {
            log.debug("REST request to get all Designers where designerIdeaDetails is null");
            return new ResponseEntity<>(designerService.findAllWhereDesignerIdeaDetailsIsNull(),
                    HttpStatus.OK);
        }
        if ("designershow-is-null".equals(filter)) {
            log.debug("REST request to get all Designers where designerShow is null");
            return new ResponseEntity<>(designerService.findAllWhereDesignerShowIsNull(),
                    HttpStatus.OK);
        }
        log.debug("REST request to get a page of Designers");
        Page<Designer> page = designerService.findAll(pageable);
    ...
    public ResponseEntity<List<Designer>> searchDesigners(@RequestParam String query, Pageable pageable) {
        ...
        Page<Designer> page = designerService.search(query, pageable);
